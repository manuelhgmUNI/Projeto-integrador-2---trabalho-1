...
int carregar_memoria_instrucoes(instrucaoSep **mem_out)
{
    ...
    for (int i = 0; i < 255; i++) {
        ...
        mem[i].opcode = (inst_bin >> 12) & 0x0F; //  0000 1111/ 15
        mem[i].rs     = (inst_bin >> 9)  & 0x07; // 00000 111/ 7
        ...
        if (mem[i].opcode == 0) {
            ...
            mem[i].tipo = (enum tipos_de_instrucao)2; //tava dando conflito por causa do i e agr deixei explicito com o casting
